...
from .models import Account, Customer, Booking, Location
...
def populate_samples():
    ...
    with transaction.manager:
        ...
        # NOTE: First Account rents out cars and transport class cars
        ...
        user = User(user_name="r1_booker", status=1, groups=[account_1],
                    email="r1_booker@random.com")
        user.set_password("r1_booker")
        user.save()

        user = User(user_name="r1_admin", status=1, groups=[account_1],
        ...
        loc_bryne = Location(
            address="Road 1",
            city="Stavanger", postal_code=4000, account=account_1)
        loc_stavanger = Location(
            address="RoadStavanger 1",
            city="Stavanger", postal_code=4035, account=account_1)
        ...
        top_cars = Category(
            resource_name="Cars",
            owner_group_id=account_1.id,
            children=sub_cars.values()).save()
        sub_transport = category_dict(["Caddy", "6m 3", "11 m3", "18 m3"], account_1.id)
        top_transport = Category(
            resource_name="Transport",
            owner_group_id=account_1.id,
            children=sub_transport.values()).save()

        focus_1 = Car(account=account_1, categories=[sub_cars["Sedan"]])
        focus_1.name = "Ford: Focus - 2012 - DF00005"
        focus_1.color = "black"

        focus_2 = Car(account=account_1, categories=[sub_cars["Sedan"]]).save()
        focus_2.name = "Ford: Focus - 2012 - DF00006"
        focus_2.color = "black"

        s60_1 = Car(account=account_1, categories=[sub_cars["Sedan"]]).save()
        s60_1.name = "Volvo: S60 - 2011 - DL00007"
        s60_1.color = "black"

        s60_2 = Car(account=account_1, categories=[sub_cars["Sedan"]]).save()
        ...
        v60_2 = Car(account=account_1, categories=[sub_cars["Stationwagon"]])
        v60_2.name = "Volvo: V60 - 2012 - DL00004"
        v60_2.color = "black"

        outlander_1 = Car(account=account_1, categories=[sub_cars["SUV"]])
        outlander_1.name = "Mitsubishi: Outlander - 2011 - DL00001"
        outlander_1.color = "black"

        outlander_2 = Car(account=account_1, categories=[sub_cars["SUV"]])
        ...
        user = User(user_name="r2_admin", status=1, groups=[account_2],
                    email="r2_admin@random.com").save()
        user.set_password("r2_admin")
        user.save()

        user = User(user_name="r2_booker", status=1, groups=[account_2],
                    email="r2_booker@random.com").save()
        user.set_password("r2_booker")
        user.save()

        user = User(user_name="steffen", status=1, groups=[account_2],
        ...
        sub_truck = category_dict(
            ...
            owner_group_id=account_2.id)
        top_truck = Category(
            resource_name="Truck",
            children=sub_truck.values(),
            owner_group_id=account_2.id).save()
        ...
        loc_bryne = Location(
            address="RoadBryne 1",
            city="Stavanger", postal_code=4000, account=account_2)
        loc_stavanger = Location(
            address="RoadStavanger 1",
            city="Stavanger", postal_code=4035, account=account_2)
        ...
        user = User(user_name="booker", status=1, groups=[account_1, account_2],
